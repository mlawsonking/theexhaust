...
**New/elevated white space (CONTESTED, not clean — see the deep pass in §13):** cross-city **311 Civic-Response Inequality**, **Insurability Retreat**, and the **Say-Do semantic gap**. A deeper prior-art sweep (132 entities, adversarially verified) found live/adjacent efforts in all three — none *occupies* the full concept, so nothing is dropped, but the "nobody does this" framing was too strong (most notably a live say-do competitor, **CivicAlign**, already exists). The single field-wide differentiator the *entire* competitive landscape still omits — confirmed across all three areas — is **the published retrocast precision/recall scorecard**. That is the moat, more than data novelty.
...
- **Single-source caveats:** domain availability is from one registrar backend (Vercel/Namecheap) — the operator must reconfirm at a registrar + USPTO TESS before buying. The medium-confidence "no live incumbent" claims for 311/insurability/say-do **have now been resolved by a dedicated deeper prior-art pass — see §13** (all three CONTESTED, adversarially verified; none occupied).
...
| I-9 | Insurability Retreat | **VIABLE-WITH-CHANGES** | SERFF/NAIC is a clickwrap anti-automation wall (**do not scrape**); re-scope to residual-market portals + per-state SERFF filings; white space CONTESTED (static national maps exist; §13.2); NAIC 2026 data call = future ground truth |
...
| I-16 | Say-Do Index | **VIABLE** | White space CONTESTED — a live competitor (CivicAlign) exists but publishes no retrocast (§13.3); differentiate on validation + tight linkage; reuses OnScript's `congress-press` corpus |
| I-17 | 311 Civic-Response Inequality | **VIABLE** | White space CONTESTED — one live fast-follower (balt311-service-equity) but no cross-city equity+receipts+retrocast (§13.1); strong local-press hook |
...
- **Cross-city 311 Civic-Response Inequality (I-17, elevated):** clean city open-data APIs; "potholes fixed in 4 days in [rich zip], 38 in [poor zip]" is pre-localized local-press candy. Strong early win. **Deep pass (§13.1): CONTESTED** — one live fast-follower (`balt311-service-equity`, Baltimore + 9 peers, cross-city *equity* still on its roadmap); differentiate on cross-city equity + receipts + retrocast (ground truth: NYC Comptroller enforcement audit, Houston Title VI finding).
- **Say-Do semantic gap (I-16, elevated):** the receipts-attached *semantic* say-vs-vote join. **Deep pass (§13.3): CONTESTED, and the "clean hole" framing is FALSE** — a live national competitor, **CivicAlign** ("Said vs. Voted"), already ships receipts-attached statement↔vote matching, so drop the "nobody does this" language; differentiate on the retrocast scorecard + tight same-bill linkage + a leading signal. Cheaper than it looked: the "say" feed is `dwillis/congress-press` — the exact corpus OnScript already runs on — joined to `unitedstates/congress` + Congress.gov + Voteview roll-calls you ingest yourself.
...
## 13. Deeper prior-art pass — the three medium-confidence white-space claims (resolved)

*Added 2026-07-11 at the operator's request, before handing to Phase 3. Method: a second multi-agent sweep — 6 blind search angles per area (GitHub, academia, govtech/insurtech product, journalism, regulator, direct-competitor), a synthesis verdict per area, then an adversarial "completeness critic" instructed to find the live incumbent the sweep missed. **132 candidate entities surveyed across the three areas; the critic confirmed the verdict HOLDS in all three.***

**Headline: all three downgrade from "clean / no-incumbent" to CONTESTED — none is OCCUPIED, so nothing is removed from the portfolio, but the optimistic "nobody does this" framing was too strong in every case.** The differentiated white space survives everywhere, and — importantly — it is *the same wedge in all three:* **the published retrocast precision/recall scorecard, which literally no live public entity in any of the three areas performs.** This strengthens rather than weakens the core finding (§8) that the retrocast scorecard is the field-wide moat.

### 13.1 Cross-city 311 inequality (I-17) — CONTESTED
- **Closest live entity:** `sadacca/balt311-service-equity` (`balt311equity.streamlit.app`, actively developed, v1.3.0 2026-06-13) — genuine within-city race/income equity for **Baltimore only**, plus a live cross-city *delivery* tab (median response / closure / on-time for Baltimore + 9 peers). But cross-city *equity* is on its own README roadmap as "still being built," with no per-record receipts and no retrocast. **A credible fast-follower to watch, not yet an occupant.** Cross-city aggregators (`311info.com`, 32 cities, monthly) publish *volume* only; live government dashboards (NYS Comptroller NYC311, Chicago median-response) are single-city or volume-only.
- **Residual white space:** live, cross-city, response-time **and** true-resolution (closed-vs-actually-fixed) sliced by income **and** race, with receipts + a retrocast scorecard + a leading signal. The retrocast differentiator is unclaimed across all 30+ entities surveyed.
- **Named retrocast ground-truth candidates (valuable):** the NYC Comptroller audit on racial gaps in building-code enforcement from 311 complaints; the Houston Title VI federal investigation into slow 311 response for Black/Hispanic residents; city-auditor reports.
- **The pitch sharpens:** the UCLA/*Governing* 7-city study found near-parity (~0.16 days per 1% income) while single-city journalism (Philadelphia) found a ~54% gap — a live, retrocast-validated cross-city index is the instrument that *settles that empirical dispute*, which is itself the hook.

### 13.2 Insurability Retreat (I-9) — CONTESTED
- More crowded than the first pass thought, still unoccupied on the full stack. **Static national non-renewal maps exist** (Public Citizen / Revolving Door Project, Brookings, PlainInsure) — all re-skins of two *frozen* government datasets (Treasury FIO ZIP-level 2018–2022; Senate Budget county-level 2018–2023). **Live trackers are single-state** (Depopulate the FAIR Plan — CA-only FAIR-plan-count proxy; CA FAIR Plan quarterly PDFs; a Wisconsin rate-filing tracker). None is live + national + receipts + retrocast.
- **This pass surfaced the retrocast ground truth the vision lacked:** the Senate Budget county dataset is mirrored **CC0 on Zenodo** (`zenodo.org/records/14829684`); CA DOI publishes an annual ZIP/county non-renewal series (captures the State Farm/Allstate exits); and crucially the **NAIC 2026 Homeowners Market Data Call** is collecting nationwide ZIP-level data with a **public report expected ~early 2027** — a future official anchor.
- **Sharpest wedge (reconciles with the SERFF ToS wall in R13):** since SERFF bulk is a clickwrap wall, build the live national feed from the only reliably-refreshing *public* signals — **state residual-market portals (CA FAIR Plan quarterly, TX/FL Citizens) + per-state SERFF withdrawal/reduction filings** — which nobody aggregates nationally, and be the only publisher of a retrocast scorecard against the named exits.

### 13.3 Say-Do semantic gap (I-16) — CONTESTED, and the "clean hole" framing is FALSE
- **The sharpest correction of the whole pass.** A live, free, public, national near-occupant already exists: **CivicAlign (`civic-align.com`) "Said vs. Voted"** — it semantically joins legislators' public statements to roll-call votes, shows each contradiction with receipts on *both* sides (statement URL + Congress.gov bill), is symmetric by design, covers Congress + 2026 candidates, and refreshes ~weekly, with real incumbents viewable without a hard paywall. **So "ProPublica Represent died and left a clean hole" is now false and must be dropped from the thesis.** (The critic re-confirmed CivicAlign as the *sole* live hit even when explicitly excluded from queries — no second occupant is hiding behind it.)
- **But CivicAlign does not occupy the retrocast-gated concept:** no published precision/recall scorecard (its trust model is "read both yourself"), only ~6-month *topic-level* stance aggregation (not tight same-bill "said X, voted literally not-X"), no leading signal, and it is a thin, unattributed AI-built freemium site. **Differentiate narrowly on validation rigor + tight linkage + a leading signal** — not on "there is no say-do tool."
- **Build-on assets (and an OnScript synergy):** the "say" feed is `dwillis/congress-press` (670k+ press releases, daily GitHub-Actions refresh) — *the exact corpus OnScript already runs on* — plus `alexlitel/congresstweets`; the "do" feed is `unitedstates/congress` + the Congress.gov API + Voteview. The canonical peer-reviewed method is **Mou et al., ACL 2021, "Align Voting Behavior with Public Statements for Legislator Representation Learning,"** and an open NLI stance model (`mlburnham/Political_DEBATE_base`) is a ready matcher building block. Dartmouth's *America's Political Pulse* is the benchmark for the "say" half.

### 13.4 Cross-cutting conclusion for Phase 3
None of the three is occupied → **no portfolio removals.** All three move from "clean white space" to **"CONTESTED — differentiate narrowly,"** and the surviving wedge is identical in every case: **the retrocast precision/recall scorecard, which no live public entity in any of the three performs.** Fable can lock these three as buildable white-space wins *on the retrocast + receipts + leading axis specifically*, cite the incumbents above as prior art / data-partners / ground truth, and — for Say-Do — delete the "nobody does this" language entirely. Two are cheaper than they looked: Say-Do reuses OnScript's `congress-press` corpus, and Insurability now has a Zenodo-mirrored ground-truth dataset plus a 2027 NAIC anchor. The three medium-confidence items from §2 are now closed; there are **no open prior-art questions left for Phase 3.**
