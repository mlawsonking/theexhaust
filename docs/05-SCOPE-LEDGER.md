...
## 5c. Publishing-surface deferrals, tied to named gates (recorded at W-007, 2026-07-29)

Raised while building the BUILD-04 launch surfaces. None was worked — scope is law — and each is recorded here rather than in a buildlog paragraph because each comes due at a *trigger*.

| Item | Status | Trigger / gate | Why deferred (and what protects us meanwhile) |
|---|---|---|---|
| **PA / WI notice extraction** | DEFERRED | a 3rd state joining them, **or** the `warn-tranche1-walled-sources` gate being decided | Both publish a link list or documents rather than a notice table, so the stdlib extractor yields nothing. **Protection is that the failure is loud and honest, not silent:** their pages say *"archived, not yet machine-readable"*, show the snapshot key and sha256, and publish **no count at all** — we never fill the gap with a guess, and the raw payload is archived on the same schedule as every other state, so nothing perishable is being lost while this waits. OH/GA (walled, deferred at W-004) belong to the same eventual parser work. |
| **WARN `parsed_rows` counts spacer rows** | RECORDED, not fixed | the next WARN collector change, or the first false volume alarm | `collectors.warn.parse_count` counts raw `<row>` tags in an `.xlsx`, so IL's spacer-padded workbook reports **408** where the sheet holds **16** real notices. Consequence is confined to the collector's own volume-anomaly band (SPEC-03 §2) — it is banding on the wrong quantity for that state, so a real IL collapse could hide inside the padding. **No published number is affected:** every figure on the site comes from `artifacts.extract`, which counts actual notices, and the two disagree openly (the state page shows 16). Fixing it means moving the collector's count onto the extractor, which couples a collector to the publishing layer — worth doing deliberately, not as a drive-by. |
| **Bot-Fight-Mode UA sensitivity (from W-001)** | STILL LATENT | any collector that stops using the framework's `http_get` | Cloudflare 403s a bare `Python-urllib` UA; the framework's honest `DEFAULT_UA` is unaffected, and every collector routes through it. Was carried as a W-007 candidate but is not a publishing-surface concern. |
