...
### W-002b · Collector state-commit-back (dedupe persistence) — `candidate` ⚑ NEW (filed by W-002, HIGH)
...
**Design decision for the orchestrator (pick one):** (a) per-collector state files `ops/state/health/<c>.json` + refactor `opscore/report.py`/`weekly.py` readers to merge them (cleanest, no write-race); (b) shared `HEALTH.json` committed by each job with a `git pull --rebase --autostash` retry loop + `[skip ci]` + `contents: write` (simplest workflow change, needs concurrency care on the shared `generated` line); (c) the SPEC-02 §2 weekly R2 session owns state commits (bounds duplicates to ≤1 week, but leaves R1 non-self-persisting). Natural fit at/with W-003. Also folds in the SPEC-02 §1 monthly keepalive-commit for the 60-day cron-disable backstop.
